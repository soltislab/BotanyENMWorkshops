...
#library(future)           # For running CANAPE in parallel processing (not needed for this demo)


## A) Area of interest (AOI) ----

# Load the Sonoran Basin and Range (EPA level 3 ecoregion)
ecoregions <- st_read("data/08_phylodiversity_phyloendemism/Ecoregions/az_eco_l3/az_eco_l3.shp")
son_ecoregion <- ecoregions %>%
                 filter(NA_L3NAME %in% c("Sonoran Basin and Range"))
...
son_states <- pol_basemap %>%
              filter(name %in% c("Arizona", "California", "Sonora", "Baja California"))

# Reproject to match ecoregion
son_states_reproj <- son_states %>%
                     st_transform(crs = st_crs(son_ecoregion))
# Add a few major cities to provide some context
locations <- data.frame(name = c("Tuscon", "Phoenix"),
...
locations_proj <- st_transform(locations_sf,
                               st_crs(son_ecoregion))
...
# Plot!
qplot <- ggplot() +
         geom_sf(son_states_reproj,
                 mapping = aes(),
                 fill = "white") +
         geom_sf(son_ecoregion,
                 mapping = aes(color = NA_L3NAME),
                 fill = "darkorange",
                 alpha = 0.2,
                 show.legend = FALSE) +
         scale_color_manual(labels = c("The Sonoran Basin and Range EPA Ecoregion"),
        ...
        # add city labels nudged
         geom_sf_text(data = locations_proj,
...
comm_matrix_xy <- as.data.frame(sdm_stack_sonoran,
                                xy = TRUE,
                                na.rm = FALSE)
...
# Save coords and assign site ids so we can join these back up later with the
# canaper output
cell_coords <- data.frame(site = paste0("cell_",
                                        seq_len(nrow(comm_matrix_xy))),
                          x = comm_matrix_xy$x,
                          y = comm_matrix_xy$y)

# Remake community matrix so that coordinate values are removed and rownames
# are the sites (necessary for canape fxns)
...
# Read in the Euphyllophyte phylogenetic tree:
# Carruthers et al. in Review. Preprint: https://doi.org/10.64898/2026.01.06.695000
...
# Removes Order Family info and reformat names to be pretty
molc_tip_df <- data.frame(original_label = molc_plant_tree$tip.label,
                          name = sub(".*_([^_]+_[^_]+)$", "\\1",molc_plant_tree$tip.label)) %>%
               mutate(name = gsub("_", " ", name))
# Assign tree tips the pretty names
molc_plant_tree$tip.label <- molc_tip_df$name

# Prune the Phylogeny to the taxa in our community matrix
# Find shared taxa among tree and community matrix
shared_sp_molc <- intersect(molc_plant_tree$tip.label,
                            colnames(comm_matrix))
# Prunes the phylogeny to these shared taxa
pruned_molc_tree <- ape::keep.tip(molc_plant_tree, shared_sp_molc)

# Now subset the community matrix so that we're only
# including taxa that are in the pruned tree
...
length(pruned_molc_tree$tip.label)
# number of tips match number of sites, so we're good to go.


## D) Visualize Cactaceae Phylogeny ----

# Prepare tree for plotting
# Set up basic tree plot using ggtree
base_tree <- ggtree(pruned_molc_tree,
                    layout = "rectangular",
                    linewidth = 0.25)
...
tree_plot <- base_tree +
             geom_tiplab(size = 2.5,
                         linesize = 0.2) +
             theme_tree2() +
             xlim(NA, max(base_tree$data$x, na.rm = TRUE) * 1.5) + # Buffer tips labels
...
### In canaper's framework, we use randomization tests where we generate a set
### of random communities, calculate the metric of interest (e.g. PD, RPD) for
### each random community, then compare the observed values to the random values.
### If the observed values are that of the extremes (2.5% for a two-tailed test),
### they can be considered more or less diverse than random.
### canaper generates random communities using the vegan package.
### To check what randomizations you can use ?vegan::commsim()
...
# while randomizing species identity. Its a good choice typically but feel free to
# try others, it can change results!
# n_iterations: make an educated guess at number of iterations needed.
# The more pixels or taxa you have, the higher this usually is
# thin: frequency to record percentage similarity between original matrix and randomized matrix.
# Pick something that makes sense with n_iterations
# seed: makes this reproducible.
iter_sim_res_molc <- cpr_iter_sim(comm = comm_matrix_molc,
...
# Visualize how many iterations are needed to arrive at a
# plateau of community dissimilarity for comparing a random community
# to the observed community
ggplot(iter_sim_res_molc, aes(x = iteration, y = similarity)) +
  geom_line() +
  theme_bw() +
  ggtitle("Number of Iterations to Arrive at Maximum Dissimilarity") +
  labs(x = "Num. iterations",
       y = "% Similarity")

# Appears that maximum dissimilarity between observed community and randomized
# happens around ~20,000 iterations.
# in regards to n_reps (number of randomized communities to simulate),
# this isn't as simple. Ideally you could do a sensitivity style analysis by
# increasing this value and checking where results converge.
# examples of how to do this in a iterative way can be found here:
# https://docs.ropensci.org/canaper/articles/canape.html
# keep in mind though that picking a very high number
# is safer at the expense of computation time, comparing more
# communities ensures you really are comparing observed vs random

# Run randomization test
#plan(multisession, workers = 7) # set up parallel computing,
# commented out for workshop. If you are using a personal computer,
# consider allocating workers to make this faster
# comm: community matrix
# phy: pruned phylogeny
# null_model: same as above!
# n_reps: number of communities, 50 is a bit slow. typically choose 500 or more (but its slow!S)
# n_iterations: number of swaps described above, based of cpr_iter_sim() output
# tbl_out: creates a tabular output
set.seed(42)
son_rand_res_molc <- cpr_rand_test(comm = comm_matrix_molc,
                                   phy = pruned_molc_tree,
                                   null_model = "curveball",
                                   n_reps = 50,
                                   n_iterations = 20000,
                                   tbl_out = TRUE)
...
son_canape <- cpr_classify_endem(son_rand_res_molc) %>%
...
basemap_xy <- st_transform(son_states,
                           crs = st_crs(sdm_stack_sonoran))
son_ecoregion_xy <- st_transform(son_ecoregion,
                                 crs = st_crs(sdm_stack_sonoran))
...
ggplot() +
  geom_tile(data = richness_df,
            mapping = aes(x = x, y = y, fill = richness)) +
  geom_sf(data = son_ecoregion_xy,
          fill = NA,
          color = "black",
          linewidth = 1) +
  geom_sf(data = son_states_reproj,
          fill = NA,
          color = "black",
          linewidth = 1) +
...
# Plot the observed phylodiversity map (compare against SR, you'll notice some
# similar trends)
ggplot() +
  geom_tile(data = son_canape_map,
            mapping = aes(x = x, y = y, fill = pd_obs)) +
  scale_fill_viridis_c(name = "Observed\nPhylogenetic Diversity",
                       option = "plasma") +
  geom_sf(data = basemap_xy,
          fill = NA,
          color = "black",
          linewidth = 1) +
  geom_sf(data = son_ecoregion_xy,
  ...
  labs(x = "Longitude",
       y = "Latitude",
       title = "Sonoran Basin and Range Cactaceae Observed Phylogenetic Diversity Map")

# Plot the significant cells for comparing the observed phylodiversity against the randomized communities
pd_sig_map <- ggplot() +
              geom_tile(data = son_canape_map,
                        mapping = aes(x = x, y = y, fill = pd_signif)) + # pd_signif contains the two-tailed test output denoting when cells are significant for PD as compared to the random communities
              ...
              labs(x = "Longitude",
                   y = "Latitude",
                   title = "Sonoran Basin and Range Cactaceae Significant Phylogenetic Diversity Map")

print(pd_sig_map)

# Plot the significant cells for comparing the alternative tree against the
# randomized communties
rpd_sig_map <- ggplot() +
               geom_tile(data = son_canape_map,
                         mapping = aes(x = x, y = y, fill = rpd_signif)) + # rpd_signif contains the two-tailed test output denoting when cells are significant for RPD as compared to the random communities
               scale_fill_manual(values = cpr_signif_cols_2,
                                 name = "Relative Phylogenetic diversity") +
               geom_sf(data = basemap_xy,
                       fill = NA,
                       color = "black") +
               geom_sf(data = son_ecoregion_xy,
               ...
               labs(x = "Longitude",
                    y = "Latitude",
                    title = "Sonoran Basin and Range Cactaceae Significant Relative Phylogenetic Diversity")
...
endemism_significant <- son_canape_map %>%
                        mutate(PE_significant = case_when(
                              # If any of the p‑upper/lower values are missing,
                              # return NA (logical)
                              is.na(pe_obs_p_upper) | #OR
                              is.na(pe_alt_obs_p_upper) |  #OR
                              is.na(rpe_obs_p_upper) | #OR
                              is.na(rpe_obs_p_lower) ~ NA, # Set as NA
                              # If either observed or alternative tree p‑upper
                              # exceeds 0.95, call it significant (1 tailed test)
                              pe_obs_p_upper > 0.95 | #OR
                              pe_alt_obs_p_upper > 0.95 ~ TRUE, # set as TRUE
                              # All other cases: not significant
                              TRUE ~ FALSE)) # set as FALSE


# Plot sites that are significant for PE in step 1
ggplot() +
  geom_tile(data = endemism_significant,
            mapping = aes(x = x, y = y, fill = PE_significant)) +
...
ggplot() +
  geom_tile(data = son_canape_map,
            mapping = aes(x = x, y = y, fill = endem_type)) + #  cpr_classify_endem() ran the two tailed test to determine endemism types, so we can just call them here
...
# Write a function that visualizes lineage composition within a site,
# and reports both the observed and expected diversity metric...
create_significant_cell_plot <- function(metric, type, cell_id,
  ...
  sig_cell_map <- sig_map +
                  ...
                  geom_segment(data = data.frame(x = start_x,
                                                 y = start_y,
                                                 xend = cell_x,
                                                 yend = cell_y),
                              mapping = aes(x = x,
  ...
  metric_label <- paste0(toupper(substr(metric, 1, 1)), substr(metric, 2, nchar(metric)),
  ...
  if (length(taxa_present_rep) > 0) {
    ...
    cell_lineages <- ggtree(tree_grouped, aes(color = group)) +
                      ...
                      geom_tiplab(size = 2.5,
                                  linesize = 0.2) +
                      theme_tree2() +
                      xlim(NA, max(base_tree$data$x, na.rm = TRUE) * 1.5) +
...
# Plot:
# Sig Low PD cell
low_pd_plot <- create_significant_cell_plot(metric = "pd",
                                            type = "low",
                                            cell_id = "cell_616",
...
# Sig High PD cell
high_pd_plot <- create_significant_cell_plot(metric = "pd",
                                            type = "high",
                                            cell_id = "cell_2722",
                                            son_canape_map = son_canape_map,
                                            comm_matrix_molc = comm_matrix_molc,
...
# Sig Low RPD cell
low_rpd_plot <- create_significant_cell_plot(metric = "rpd",
                                             type = "low",
                                             cell_id = "cell_507",
                                             son_canape_map = son_canape_map,
                                             comm_matrix_molc = comm_matrix_molc,
...
# Sig High RPD cell
high_rpd_plot <- create_significant_cell_plot(metric = "rpd",
                                              type = "high",
                                              cell_id = "cell_2050",
                                              son_canape_map = son_canape_map,
                                              comm_matrix_molc = comm_matrix_molc,
